...
// Муравей / Lifan KP230 badge
// Designed for clean FDM printing with a 0.4 mm nozzle.
// Overall target size: 160 mm wide, ~57 mm high, 3.0 mm total thickness.
...
module plaque_raw2d() {
    union() {
        circle(r=center_r);
        wing_left_raw();
        mirror([1,0,0]) wing_left_raw();
    }
}

module plaque2d() {
    offset(r=0.9) offset(delta=-0.9) plaque_raw2d();
}

module panel2d() {
    offset(delta=-2.15) plaque2d();
}

module frame2d() {
    difference() {
        plaque2d();
        offset(delta=-frame_w) plaque2d();
    }
}

module capsule(p1, p2, d=1.4) {
    hull() {
...
module polyline(points, d=1.4) {
    for (i=[0:len(points)-2]) capsule(points[i], points[i+1], d);
}
...
module gear2d() {
    difference() {
        union() {
            ...
            for (a=[0:30:330])
                rotate(a) translate([22.5,0]) square([6.0,5.2], center=true);
        }
        circle(r=18.25);
    }
}
...
module stripe_pair(side=1) {
    x1 = 32.0 * side;
    x2 = 68.5 * side;
    for (y=[12.2,14.55,-12.2,-14.55])
        capsule([x1,y],[x2,y],1.05);
}
...
module left_text2d() {
    translate([-49.5,-0.2])
        scale([0.47,1.0])
            offset(delta=0.12)
                text("МУРАВЕЙ", size=12.6, font=font_main,
                     halign="center", valign="center", spacing=0.96);
}
...
module right_text2d() {
    translate([48.8,-0.25])
        scale([0.42,1.0])
            offset(delta=0.18)
                text("Lifan KP230", size=11.7, font=font_right,
                     halign="center", valign="center", spacing=0.92);
}
...
module through_holes(hh) {
    for (x=[-hole_x,hole_x])
        translate([x,0,-0.2]) cylinder(h=hh+0.4,d=hole_d);
}
...
module combined_part() {
    difference() {
        union() {
            linear_extrude(height=base_h, convexity=10) plaque2d();
            translate([0,0,base_h-combined_overlap])
                linear_extrude(height=relief_h+combined_overlap, convexity=20)
                    silver_details2d();
        }
        through_holes(base_h+relief_h);
    }
...
if (part == "combined") combined_part();
    ...
    color([0.04,0.04,0.04]) black_part();
    color([0.75,0.76,0.78]) silver_part();
}
